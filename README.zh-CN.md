...
EventHorizon 是一个基于 .NET 的编码代理宿主，提供 Engine API、嵌入式 Workbench 资源、会话持久化、Provider 动态切换、MCP 集成和 Skill 导入能力。

## 特性

- EventHorizon Runtime 与 Engine 共用一个 Host
...
服务会根据 `src/EventHorizon/appsettings.json` 里的 `Kestrel:Endpoints:EventHorizon:Url` 监听，并提供：

- `api/*` Controller API
- 嵌入式静态资源页面
- `/api/sessions/{sessionId}/runs/{runId}/events` 用于 Run 事件流
