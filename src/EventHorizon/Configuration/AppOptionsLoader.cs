...
internal sealed class AppOptionsInitializer : IAppOptionsInitializer
{
    ...
    public AppOptionsInitializer(IPathEnvironment pathEnvironment)
    {
    ...
    {
        if (options.Providers.Count == 0 && HasConfiguredValues(options.Provider) && !options.Providers.ContainsKey("default"))
        {
